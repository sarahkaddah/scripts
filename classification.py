...
def classification(kmer_table, lda_sample_size, verbose=0):
	...
	while(queue):
        ...
        #####modif3
	    if verbose > 4:
		    print("   Nb lignes et nb colonnes {}".format(pca.shape))
        #####
        ...
        ####modif1    
	    clusters, clust_time = compute_clustering_fast(distance)
	    if verbose > 4:
	        print("   Cluster time: %f s" % (clust_time))
	    ####
...
def get_kmer_table(kmer_file, verbose=1):
    ...
    #########modif2
    kmer_table_file = str (kmer_table)    
    kwrite=open("kmers_tableau.txt","w")
    kwrite.write(kmer_table_file)
    kwrite.close()
    #print("Nb colonnes: {}\n".format(nb_col))
    print("Nb lignes et colonnes: {}\n".format(kmer_table.shape))
    ##########
    if verbose > 0:
...
def compute_clustering_fast(distance):
    ...
    t2=time.clock()
    t3=t2-t1
    return scipy.cluster.hierarchy.fcluster (c, 2, criterion="maxclust"), t3
